...
UTC_STOCK_MARKET_HOURS = [1430,2100]
...
class StockSymbol:
    ...
    def date(self, new_date):
        try:
            d = datetime.strptime(new_date, self.valid_date_format)
        except ValueError:
            self._date = False
...
def create_stock_prices_layout(symbols: list[StockSymbol]):
...
async def main():
    ...
    async with aiohttp.ClientSession(headers=headers) as session:
        while True:
            ...
            stock_prices_window = sg.Window(f"Stock Prices for {cur_date.date()}", stock_prices_layout)
